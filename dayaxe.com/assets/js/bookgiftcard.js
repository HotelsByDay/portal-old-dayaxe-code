...
function checkIsValid() {
    ...
    $('.input-group input').each(function () {
        if ((!$(this).hasClass("discount-text") && !$(this).hasClass('datepicker') && $(this).val() == '')
            || ($(this).hasClass('datepicker') && $(this).val() == '')) {
            emptyText = true;
        }
    });

    if ($('#UseNewCard').length <= 0 || emptyText) {
...
function domReady() {
    ...
    $('.input-group input, .input-group textarea').keyup(function () {
        var empty = false;
        var isValidZip = true;
        var txtzipcode = $("#txtzipcode");
        var purchaseButton = $("#AnchorButton");
        ...
        $('.input-group input, .input-group textarea').each(function () {
            if ($(this).attr('id') === 'ToText') {
                $.jCookie('EmailRecipient', $(this).val());
            }
            if ($(this).attr('id') === 'NameText') {
                $.jCookie('NameRecipient', $(this).val());
            }
            if ($(this).attr('id') === 'MessageText') {
                $.jCookie('MessageRecipient', $(this).val());
            }
            if ((!$(this).hasClass("discount-text") && !$(this).hasClass('datepicker') && !$(this).hasClass('message') && $(this).val() == '')
                || ($(this).hasClass('datepicker') && $(this).val() == '')) {
                empty = true;
            }
        });
    ...
    $(document).on('click', '#AnchorButton', function () {
        ...
        $('#HidUserBookedDate').val(stamp);

        // Clear Cookie
        $.jCookie('EmailRecipient', null);
        $.jCookie('NameRecipient', null);
        $.jCookie('MessageRecipient', null);
        $.jCookie('DeliveryDate', null);
        $(".hoteloverlaywrap").show();
        $(".hotelmodal").css({ "padding-left": 10, "padding-right": 10 })
            .html("SIT TIGHT...WE'RE PURCHASING<br/> YOUR GIFT CARD<div class=\"imgspin text-center\"><img src=\"/images/loading.gif\" alt=\"\"></div>")
            .show();
    });
    ...
    if ($('#ToText').val() == '' && $.jCookie('EmailRecipient')) {
        $('#ToText').val($.jCookie('EmailRecipient'));
    }

    if ($('#NameText').val() == '' && $.jCookie('NameRecipient')) {
        $('#NameText').val($.jCookie('NameRecipient'));
    }

    if ($('#MessageText').val() == '' && $.jCookie('MessageRecipient')) {
        $('#MessageText').val($.jCookie('MessageRecipient'));
    }

    if ($('#DeliveryDateText').val() == '' && $.jCookie('DeliveryDate')) {
        $('#DeliveryDateText').val($.jCookie('DeliveryDate'));
    }

    if ($.jCookie('DeliveryDate')) {
        $('#checkindate').datepicker('setDate', new Date($.jCookie('DeliveryDate')));
    }

    // Check if sign in after enter info
    checkIsValid();
}
...
function pageLoad(sender, args) {
    ...
    $('#checkindate').datepicker({
        ...
        $.jCookie('DeliveryDate', $('#checkindate input').val());
    });
